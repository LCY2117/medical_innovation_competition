...
- Android 首页和事件页已接通“自动接单”，登录后可直接加入当前事件并进入任务页；自动接单成功后会立即刷新事件快照，避免等待 WebSocket 首帧才显示 JOINED/任务状态。
...
输出目录会包含 `round-summary.csv`、`round-analysis.md`、`round-chart-data.csv` 和 `round-review-actions.csv`。`round-summary.csv` 会附带每轮证据质量等级、质量分、critical/warning/info 数量、缺失关键节点和提示代码；`round-analysis.md` 会汇总证据质量分布，并用“需复核轮次”表先筛掉需要重跑或人工补充说明的轮次；`round-review-actions.csv` 会把每轮标成可采用、带备注采用、需重跑/人工补充或暂不使用。
...
继续生成 PPT 安全口径的 Markdown 分析摘要，也可以同步生成图表底表和复核行动清单：
