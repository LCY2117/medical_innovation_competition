...
- [x] Web 增加“预实验记录包”下载入口：事件 JSON、匿名化 JSON/CSV、说明、专家摘要、专家复核清单、观察员记录表、单轮汇总表和 manifest 校验清单。
- [x] Android 本地归档可展示参与者视角的任务总结。
- [x] 补充专家反馈模板中的证据包审阅材料、系统截图清单和 3-5 分钟演示脚本。
...
- 预实验证据包包含匿名化 JSON/CSV、专家摘要、专家复核清单、观察员记录表、单轮汇总表和 manifest 校验，外部材料默认使用匿名化文件。
